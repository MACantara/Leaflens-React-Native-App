...
const STANDARD_NON_PLANT_RESPONSE: LeafAnalysisResponse = {
  ...
  confidenceScore: 0,
  confidenceLabel: 'Low',
  keyCharacteristics: ['Image does not appear to contain recognizable plant material.'],
  careTips: 'Retake the photo with one clear plant part in frame (leaf, stem, flower, fruit, or root) and good lighting.',
  safetyNotes: 'Do not consume unknown specimens without expert confirmation.',
  identificationNotes: 'The uploaded image does not appear to be a plant photo.',
...
export async function analyzeLeafImage(image: Buffer, mimeType: string): Promise<LeafAnalysisResponse> {
  ...
  const prompt = [
    'Analyze this plant image and provide the information in the following JSON format.',
    'IMPORTANT: Return ONLY the raw JSON object. Do NOT include markdown code blocks, backticks, or any explanation text.',
    'Valid plant images include leaves, stems, flowers, fruits, seeds, roots, and tubers (for example, kamote/sweet potato).',
    'Do not classify an image as non-plant solely because the visible plant part is not a leaf.',
    'If the image does not contain recognizable plant material (for example, object, person, animal, document, or unclear scene), still return the same JSON schema using this exact fallback intent:',
    '- commonName: "Not a plant"',
    '- scientificName: "Not a plant"',
    '- origin: "N/A"',
    '- uses: "N/A"',
    ...
    '  "commonName": "common name here (include Tagalog name when known)",',
    ...
    'For "commonName", include the widely used Tagalog/Filipino local name when known.',
    'Format example: "Guava (Bayabas)" or "Lemongrass (Tanglad)". If a Tagalog name is unclear, keep the best known common name.',
    ...
    'For "keyCharacteristics", provide 2-4 concise observations visible in the plant image.',
